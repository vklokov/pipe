  ...
  class Entity
    class << self
      ...
      def search(params)
        ...
        field_key  = schema.values.include?(key) ? schema.find { |_, v| v == key }[0] : key
        request    = Pipe::Routes::ROUTE_ITEM_SEARCH
        response   = Pipe::Client.new(
          request, term: term, field_key: field_key, field_type: field_type, return_item_ids: true
        ).get
    ...
    def define_attributes
      serialized_keys = Serializer.new(
        @source, self.class.fields, self.class.schema
      ).serialize
